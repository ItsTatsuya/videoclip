...
function this.toms(timestamp)
    return h.format_timestamp(timestamp)
end
...
function this.video_filter_chain(config)
    local filters = {}
    if not (config.video_width == -2 and config.video_height == -2) then
        table.insert(filters, table.concat { 'scale=', config.video_width, ':', config.video_height })
    end
...
function this.run_tests()
    ...
    h.assert_equals(this.video_filter_chain({ video_width = -2, video_height = -2, video_fps = 'auto' }), { 'format=yuv420p' })
    h.assert_equals(this.video_filter_chain({ video_width = -2, video_height = 480, video_fps = 'auto' }), { 'scale=-2:480', 'format=yuv420p' })
    ...
    h.assert_equals(this.audio_codec_to_extension("vorbis"), ".ogg")
    h.assert_equals(this.audio_codec_to_extension("ac3"), ".ac3")
    h.assert_equals(this.audio_codec_to_extension("eac3"), ".eac3")
    h.assert_equals(this.audio_codec_to_extension("flac"), ".flac")
